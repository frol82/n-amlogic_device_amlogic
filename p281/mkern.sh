...
#ROOTFS=$1
ROOTFS="out/target/product/p281/ramdisk.img"
...
KERNEL_OUT=out/target/product/p281/obj/KERNEL_OBJ
#mkdir -p $KERNEL_OUT
...
if [ "$2" != "m" ]; then
    make -C common O=../$KERNEL_OUT gxl_p281_1g.dtb ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE PARTITION_DTSI=partition_mbox.dtsi
    make -C common O=../$KERNEL_OUT gxl_p281_2g.dtb ARCH=arm64 CROSS_COMPILE=$PREFIX_CROSS_COMPILE PARTITION_DTSI=partition_mbox.dtsi
fi
...
if [ "$2" != "m" ]; then
    out/host/linux-x86/bin/mkbootimg --kernel common/../$KERNEL_OUT/arch/arm64/boot/Image.gz \
        ...
        --ramdisk ${ROOTFS} \
        --output ./out/target/product/p281/boot.img
    ls -l ./out/target/product/p281/boot.img
    echo "boot.img done"
fi
